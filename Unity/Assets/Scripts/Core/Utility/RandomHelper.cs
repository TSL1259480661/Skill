...
public static class RandomHelper
{
	...
	public static bool isInitUnityRandom = false;
	...
	//0-100的概率随机
	public static bool RandProbality(int probality)
	{
		if (probality >= 100)
			return true;
		if (probality <= 0)
			return false;
		return RandomNumber(1, 101) <= probality;
	}
	...
	private static float UnityRange(float max)
	{
		if (isInitUnityRandom == false)
		{
			UnityEngine.Random.InitState(Guid.NewGuid().GetHashCode());
			isInitUnityRandom = true;
		}
	...
	public static int RandomByWeight(float[] weights)
	{
		...
		float number_rand = UnityRange(sum);
