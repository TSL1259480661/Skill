...
namespace ET
{
    public static class GameObjectPoolHelper
    {
        ...
        public static void InitPoolFormGamObject(string poolName, GameObject pb, int size, PoolInflationType type = PoolInflationType.INCREMENT)
        {
            if (poolDict.ContainsKey(poolName))
            {
                return;
        ...
        public static void ReturnAllChild(Transform parent)
        {
            for (int i = 0; i < parent.childCount; i++)
            {
                ...
                {
                    ReturnAllChild(child);
                }
                if (child.GetComponent<PoolObject>())
                {
                    ReturnTransformToPool(child);
                    i--;
                }
            }
        }
