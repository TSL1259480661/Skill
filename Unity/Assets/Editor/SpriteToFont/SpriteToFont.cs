...
public class SpriteToFont : MonoBehaviour
{
    ...
    static void CreateMyFontSprite()
    {
        ...
        //if (selectionPath.Contains(resoursePath))
        {
			...
			Object[] objArry = AssetDatabase.LoadAllAssetsAtPath(selectionPath);
			List<Sprite> sprites = new List<Sprite>();
			foreach (var item in objArry)
			{
				if (item is Sprite)
				{
			...
			if (sprites.Count > 0)
            {
                ...
                AssetDatabase.Refresh();//刷新资源，在Mac上不起作用，由于上面fresh之后在编辑器中依然没有刷新  
            ...
            //else
            //{
            //    Debug.LogWarning("没有选中Sprite文件，需要将Sprite放到Resources文件夹下面");
            //}
        }
    }
}
